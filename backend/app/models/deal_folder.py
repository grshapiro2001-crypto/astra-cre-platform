...
class BOVPricingTier(Base):
    ...
    # Loan assumptions
    leverage = Column(Float, nullable=True)  # LTV %
    loan_amount = Column(Integer, nullable=True)
    interest_rate = Column(Float, nullable=True)  # 5.25
    io_period_months = Column(Integer, nullable=True)
    amortization_years = Column(Integer, nullable=True)

    # Return metrics
    unlevered_irr = Column(Float, nullable=True)
    levered_irr = Column(Float, nullable=True)
    equity_multiple = Column(Float, nullable=True)
    avg_cash_on_cash = Column(Float, nullable=True)

    # Terminal assumptions
    terminal_cap_rate = Column(Float, nullable=True)
