...
Talisman IO branded 3-page Property Summary
...
def _ensure_fonts():
    """Download and register Talisman fonts. Falls back to Helvetica on failure."""
...
def _draw_logo(c, logo_x, logo_y):
    """Draw Talisman compass rose logo (Fix 2)."""
    logo_s = 18
...
def _page_header(c, prop_name, score=None):
    ...
    # "Talisman" text in Syne-Bold
    c.setFillColor(PURPLE_LIGHT)
    c.setFont(_font("Syne-Bold"), 16)
    c.drawString(MARGIN_L + 24, header_y, "Talisman")

    # "IO" in lighter color
    talisman_w = c.stringWidth("Talisman", _font("Syne-Bold"), 16)
    c.setFillColor(TEXT_MUTED)
    c.setFont(_font("Syne-Bold"), 16)
    c.drawString(MARGIN_L + 24 + talisman_w + 4, header_y, "IO")

    # Property name
    logo_text_w = 24 + talisman_w + 4 + c.stringWidth("IO", _font("Syne-Bold"), 16) + 16
...
def _page_footer(c, page_num, prop_name):
    ...
    c.setFillColor(TEXT_MUTED)
    left_text = "Generated by Talisman IO " + EM_DASH + " " + date_str
...
def generate_deal_summary(property_id: int, db: Session) -> bytes:
    ...
    """
    # Ensure Talisman fonts are registered (Fix 1)
    _ensure_fonts()
